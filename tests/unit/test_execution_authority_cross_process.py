...
spec_set = build_instrument_spec_set(InstrumentSpecSetId("phase1.cross-oms.v1"), items)
...
execution = create_phase1_order_authority(
    run_id=run_id,
    spec_set=spec_set,
    execution_policy=execution_policy,
    risk_policy=policy,
...
first_intent = intent(aapl.instrument, 1, "2")
second_intent = intent(ibm.instrument, 2, "5")
first_result = risk.evaluate(first_intent, snapshot)
second_result = risk.evaluate(second_intent, snapshot)
first = execution.create_order(first_intent, first_result)
second = execution.create_order(second_intent, second_result)
replay = execution.create_order(first_intent, first_result)

vectors = {
    "orders": [
        [canonical_order_bytes(order).hex(), order_digest(order).value]
    ...
    "replay_identity": replay is first,
    "next_order_sequence": execution.next_order_sequence,
}
...
def test_order_authority_vectors_ignore_process_environment_and_input_order(
    tmp_path: Path,
) -> None:
