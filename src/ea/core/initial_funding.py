...
INITIAL_FUNDING_SPEC_SCHEMA = "ea-initial-funding-spec-v1"
    ...
    def __post_init__(self) -> None:
        ...
        if self.result is InitialFundingResult.APPLIED:
            ...
            if (
                ...
                or transaction.manifest_sha256 != self.manifest_sha256
                or transaction.funding_spec_sha256 != self.submitted_funding_spec_sha256
                or transaction.prepared_audit_acknowledgement_sha256
                != self.prepared_audit_acknowledgement_sha256
                or transaction.ledger_sequence != self.after_snapshot_version
                or transaction.entry_id != self.snapshot.last_entry_id
                or initial_funding_transaction_digest(transaction)
                != self.snapshot.last_transaction_sha256
            ):
                raise _fail("applied funding outcome transaction conflicts")
        elif (
