    ...
    try:
        return LineageSpecV2(
            ...
            runtime=InstalledRuntimeSpecV2(
                ea_distribution=ea_distribution,
...
def read_manifest_v2(data: bytes) -> RunManifestV2:
    """Strictly parse a v2 manifest without accepting a v1 shape."""
    root = cast(
        _ManifestWire,
        _require_exact_keys(
            _strict_json_loads(data),
...
def read_manifest(data: bytes) -> RunManifest | RunManifestV2:
    """Version-dispatch a canonical historical or installed-product manifest."""
    root = _require_exact_keys(
        _strict_json_loads(data),
        frozenset({"lineage_sha256", "manifest_schema_version", "run_id", "spec"}),
        field="manifest",
    )
    version = root["manifest_schema_version"]
    if version == 1:
